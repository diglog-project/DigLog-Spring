...
public class PostCreationNotificationStrategy implements NotificationStrategy {
	...
	private static final String NOTIFICATION_MESSAGE_FORMAT = "%s님이 \"%s\" 게시글을 작성했습니다.";
	...
	public List<Member> resolveReceivers(UUID dataId) {
		Member author = searchPostBy(dataId).getMember();

		List<Subscription> subscriptions =
			subscriptionRepository.findAllByAuthorAndSubscriberIsDeletedFalse(author);

		return subscriptions.stream()
			.filter(Subscription::isNotificationEnabled)
			.map(Subscription::getSubscriber)
			.filter(subscriber -> subscriber.isDifferent(author))
			.toList();
	}
